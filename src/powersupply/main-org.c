...
SIGNAL(SIG_USART_DATA)    // Received Data from PC
{
  if (SerOutBuffIn!=SerOutBuffOut)
  {
	...
	if (SerOutBuffOut>=SerBufMax-1) SerOutBuffOut=0; else SerOutBuffOut++;
  }
   else UCSR0B=UCSR0B & (255-32);
}
...
uint8_t GetDataIn()
{
  ...
  if (NewDataInAvailable()==1)
  {
	...
	if (SerInBuffOut>=SerBufMax-1) SerInBuffOut=0; else SerInBuffOut++;
	return Temp;
  }
   else return(0);
}
...
void SendByte(uint8_t Data)
{
  ...
  if (SerOutBuffIn>=SerBufMax-1) SerOutBuffIn=0; else SerOutBuffIn++;
  sei();
  UCSR0B=UCSR0B | 32;
}
...
{
  if ((PINC & 16)==0) return(0); else return(1);
}
...
{
  if ((PINC & 1)==1) return(1); else return(0);
}
