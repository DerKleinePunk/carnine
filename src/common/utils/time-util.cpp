...
static char* format_timestamp_internal(char* buf, size_t l, usec_t t, bool utc, bool us)
{
    ...
    if(t <= 0 || t == USEC_INFINITY) return NULL; /* Timestamp is unset */

    /* Let's not format times with years > 9999 */
    if(t > USEC_TIMESTAMP_FORMATTABLE_MAX) return NULL;

    sec = (time_t)(t / USEC_PER_SEC); /* Round down */

    if(!localtime_or_gmtime_r(&sec, &tm, utc)) return NULL;
    ...
    if(strftime(buf + 3, l - 3, " %Y-%m-%d %H:%M:%S", &tm) <= 0) return NULL; /* Doesn't fit */

    /* Append the microseconds part, if that's requested */
    if(us) {
        n = strlen(buf);
        if(n + 8 > l) return NULL; /* Microseconds part doesn't fit. */
    ...
    if(utc) {
        ...
        if(n + 5 > l) return NULL; /* "UTC" doesn't fit. */
...
usec_t timespec_load(const struct timespec* ts)
{
    ...
    if(ts->tv_sec < 0 || ts->tv_nsec < 0) return USEC_INFINITY;
