        ...
        for (int i = 1; i < argc; ++i) {
            ...
            if (v != nullptr && strlen(v) > 0) {
                ...
                if (HasParamWithValue(key)) {
                    LOG(WARNING) << "Skipping [" << key << "] arg since it already has value [" << GetParamValue(key) << "]";
                } else {
                    LOG(DEBUG) << "add Commnadline Parameter " << key << " Value <" << std::string(v + 1) << ">";
            ...
            if (v == nullptr) {
                ...
                if(key.size() > 0) {
                    if (HasParam(key)) {
                        LOG(WARNING) << "Skipping [" << key << "] arg since it already exists";
                    } else {
                        LOG(DEBUG) << "add Commnadline Parameter " << key << " Key Only";
